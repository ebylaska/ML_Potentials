...
sigmoidp  = lambda x: 1.0-math.tanh(x)**2
sigmoidpp = lambda x: -2.0*math.tanh(x)*(1.0-math.tanh(x)**2)

nn_machine  = myfeedforward3.MyFeedForward([nparameters,nparameters,nparameters,nparameters,nparameters,1],[sigmoid,sigmoid,sigmoid,sigmoid,sigmoid,lambda x: x],[sigmoidp,sigmoidp,sigmoidp,sigmoidp,sigmoidp,lambda x: 1], [sigmoidpp,sigmoidpp,sigmoidpp,sigmoidpp,sigmoidpp,lambda x: 0])
nn_weights  = []
for ii in range(nion):
   nn_weights += nn_machine.initial_w()

nn_weights0 = nn_weights[:]
...
for i in range(len(nn_weights)):
   nn_weights[i] -= nn_weights0[i]
print("diff nn_weights=",nn_weights)
...
for it in range(100):
   ...
   errorall = 0.0
   errorall2 = 0.0
   if ((it%10)==0): print("     frame  error  Eguess  Exact\n"+"      ----------------------")
   for frame in range(nframes):
      etmp = []
      dedw = []
