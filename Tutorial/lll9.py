...
def plote(plot,machine,weights):
   ...
   for i in range(-19,20):
      x = i/19.0
      e0 = math.sin(x*y0*u0) + math.cos(2*x*y0*u0)**2 + math.sin(x+y0+u0)
      e1 = math.sin(x*y1*u1) + math.cos(2*x*y1*u1)**2 + math.sin(x+y1+u1)
      e2 = math.sin(x*y2*u2) + math.cos(2*x*y2*u2)**2 + math.sin(x+y2+u2)
      x0.append(x)
      z0.append(e0)
      z1.append(e1)
      z2.append(e2)
      ee0 = machine.evaluate([x,y0,u0],weights)[0]
      ee1 = machine.evaluate([x,y1,u1],weights)[0]
      ee2 = machine.evaluate([x,y2,u2],weights)[0]
      zz0.append(ee0)
      zz1.append(ee1)
      zz2.append(ee2)
      if (e0 <emin): emin = e0
...
for i in range(1000000):
   ...
   us = 2.0*random.random()-1.0
   es = math.sin(xs*ys*us) + math.cos(2*xs*ys*us)**2 + math.sin(xs+ys+us)
   if ((i%1000)==0):
