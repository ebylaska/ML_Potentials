...
def plote(plot,xdat,edat,machine,weights,xmin,xmax,emin,emax):
   ...
   for i in range(len(xdat)):
      ...
      e1  = es1*(emax-emin) + emin
      x1.append(x)
      y.append(e)
      y1.append(e1)
   plot.resetwindow(xmin,emin-de,xmax,emax+de,"Spring Energies")
...
bias = []
machine = myfeedforward.MyFeedForward([1,15,30,1],[sigmoid,sigmoid,sigmoid,sigmoid],[sigmoidp,sigmoidp,sigmoidp,sigmoidp],[sigmoidpp,sigmoidpp,sigmoidpp,sigmoidpp],bias)
...
plote(plot,xdat,edat,machine,weights,xmin,xmax,emin,emax)
...
for i in range(1000000):
   ...
   if ((i%10000)==0):
      es1 = machine.evaluate([xs],weights)[0]
      e1  = es1*(emax-emin) + emin
      print "%10d %10.3f %10.3f %10.3f  ||  %8.3f %8.3f %12.6f" % (i,x,e,e1,xs,es1,error)
      plote(plot,xdat,edat,machine,weights,xmin,xmax,emin,emax)
