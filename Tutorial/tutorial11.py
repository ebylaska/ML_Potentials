...
def read_ml_urlfile(urlfilename):
   ...
   while (framecounter<nframes):
      ...
      energies = [eval(x) for x in line.split("outputLayer:")[1].split(":outputLayer")[0].split()]
      framecounter += 1

      yield (id,xdata,energies)
...
#
   ...
   Usage: tutorial11 -f nn_file.dat -n hidden layers -b nbatch -p nepoch
   ...
   nbatch = 25
   nepoch = 100
   mlfilename = "nnlsq_chno_b3lyp.dat"
   ...
   xinputs  = []
   ids      = []
   for (id,xdata,energylist) in read_ml_urlfile(mlfilename):
      energy = energylist[0]
      #print("nframes=",nframes)
   ...
   with open(param_filename,'wb') as ff:
      ff.write(pickle.dumps((ninput,nframes,demin,demax,demid,dedif,B)))


   scaled_energies = [0.0]*nframes
   for i in range(nframes):
      scaled_energies[i] = (delta_energies[i]-demin)/(2*dedif)
