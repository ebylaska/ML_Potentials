...
#                                            #
##############################################
def dataplot_pathenergy(datafile,y0,machine,weights,ds1,ds2,ds3):
   #colors = ("blue","green","yellow","purple","orange")
   #ncc    = len(colors)
   delta = 0.1
   ymin = +999999.9
   ...
   ### reset the plotting window and then plot data ###
   data = "#Title Comparison of path energies\n"
   data += "#Labels frame  E(exact) E(predicted)\n"
   for i in range(len(y0)):
      data += "%d %e %e\n" % (i,y0[i],y1[i])
   with open(datafile,"w") as ff:
      ff.write(data)
...
dataplot_pathenergy("tutorial5b.dat",energies[nframes0:],machine,weights,dist1[nframes0:],dist2[nframes0:],dist3[nframes0:])
